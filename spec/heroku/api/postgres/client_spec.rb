...
RSpec.describe Heroku::Api::Postgres::Client, :vcr do
  ...
  describe '.connect_oauth' do
    ...
    it {
      expect do
        Heroku::Api::Postgres.connect_oauth(oauth_token).databases.info(database_id)
  ...
  describe '.connect' do
    ...
    it { expect { Heroku::Api::Postgres.connect(api_key).databases.info(database_id) }.not_to raise_error }
  end
end
